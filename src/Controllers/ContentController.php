...
class ContentController extends Controller
{
    ...
    {   
        ...
        return Admin::content(function (Content $content) use ($mid) {
            ...
            if($mid>0 or $mid=='all')$content->body($this->grid($mid));
            ...
            else $content->body(NetMenu::tree(function ($tree) {
                $tree->isList();
                $tree->branch(function ($branch) {
                    ...
                    return "<a href='/admin/setting/content?id={$branch['id']}'>{$branch['id']} - {$branch['title']} [$coden] （共 $countInfo 条）".($branch['st']?'':'[关闭]')." ".($branch['is_url']?'[外链]':'')."</a>";
                });
    ...
    protected function grid($mid=0)
    {
        ...
        return Admin::grid(NetContent::class, function (Grid $grid) use ($mid) {
            ...
            $grid->column('category_id','栏目')->editable('select',NetMenu::selectOptions());
        ...
        return Admin::form(NetContent::class, function (Form $form) {
            ...
            $form->saving(function (Form $form) {
                ...
                if(config('audit.switch','off')=="on"){//审核
                    ...
                    $menu=NetMenu::find($form->category_id);
            ...
            $form->tab('基础内容设置', function ($form) {
                $id = @$_GET['id'];
                $form->select('category_id', '栏目')->options(NetMenu::selectOptions())->default($id);
                $form->text('title', '标题');
